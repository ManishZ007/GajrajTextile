...
export default function ProductsEdit() {
  ...
  useEffect(() => {
    reset();
    Promise.all([fetchProductById(productId), fetchCategories()])
      .then(([product, cats]) => {
        populateForm(product);
        setCategories(cats);
        // Populate existing default config if present
        if (product.defaultPadarId || product.defaultBorderId) {
          setDefaultConfig({
            defaultPadarId: product.defaultPadarId ?? "",
            defaultBorderId: product.defaultBorderId ?? "",
            defaultButtiId: product.defaultButtiId ?? "",
            defaultBodyColorId: product.defaultBodyColorId ?? "",
            defaultBorderColorId: product.defaultBorderColorId ?? "",
            defaultZari: product.defaultZari ?? "",
          });
        }
      })
  ...
  useEffect(() => {
    return () => {
      images.forEach((img) => {
        if (!img.uploaded) URL.revokeObjectURL(img.preview);
      });
    };
  }, []);

  // When category changes, fetch category + customization options
  useEffect(() => {
    if (!categoryId) {
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      const imageData = await uploadNewImages();
      ...
      await updateProduct(productId, {
        ...
        variants:
          variants.length > 0
            ? variants
                .filter((v) => v.color || v.size || v.sku || v.price)
                .map((v) => ({
                  size: v.size,
                  color: v.color,
  ...
  return (
    <div className="flex flex-col gap-4 min-h-full">
      ...
      {/* Main content */}
      <div className={`grid gap-4 ${showPreview ? "grid-cols-[1fr_auto]" : "grid-cols-1"}`}>
        {/* Left: form */}
            ...
            <div className="sticky top-6 bg-white/40 backdrop-blur-sm border border-white/50 rounded-2xl overflow-hidden flex flex-col">
              ...
              <div className="relative bg-gray-100 aspect-square flex items-center justify-center overflow-hidden">
                ...
                {images.length > 1 && (
                  <>
                    ...
                    <button
                      onClick={() => setPreviewIndex((p) => (p + 1) % images.length)}
                      ...
                      {images.map((_, d) => (
                        <span
                          ...
                          className={`w-1.5 h-1.5 rounded-full ${
                            d === previewIndex ? "bg-gray-700" : "bg-gray-300"
                          }`}
                        />
                      ))}
                    </div>
              ...
              <div className="p-4 flex flex-col gap-2.5">
                ...
                <p className="text-lg font-bold text-gray-800">
                  {basePrice ? `₹${parseFloat(basePrice).toLocaleString("en-IN")}` : "₹0"}
                </p>
