  ...
  return (
    ...
    >
      <div className="flex items-center justify-center w-7 h-7 rounded-lg bg-gray-100 shrink-0">
        {icon && <span className="text-gray-500"><IconOrders /></span>}
        {dot && <span className={`w-2 h-2 rounded-full ${dot}`} />}
      </div>
      <div>
        <p className="text-lg font-bold text-gray-800 leading-none">{count}</p>
        <p className="text-[11px] text-[#616a7c] mt-0.5 whitespace-nowrap">{label}</p>
      </div>
    </button>
  );
}
...
export default function OrdersAll() {
  ...
  async function fetchData() {
    setLoading(true);
    try {
      const params: Parameters<typeof fetchAllOrders>[0] = { page, size: PAGE_SIZE };
  ...
  function changeStatus(s: StatusFilter) { setStatusFilter(s); setPage(0); }
  function changeType(t: TypeFilter)     { setTypeFilter(t);   setPage(0); }
  ...
  async function handleCancel(e: React.MouseEvent, orderId: string) {
    ...
    await fetchData();
    // Refresh counts
    fetchAllOrders({ size: 1000 }).then((data) => {
      const all: Order[] = data.content ?? [];
      const c: Record<string, number> = { ALL: all.length };
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-1">
        <StatCard label="All"         count={counts["ALL"]         ?? 0} icon       active={statusFilter === "ALL"}          onClick={() => changeStatus("ALL")} />
        <StatCard label="Pending"     count={counts["PENDING"]     ?? 0} dot="bg-amber-400"   active={statusFilter === "PENDING"}      onClick={() => changeStatus("PENDING")} />
        <StatCard label="Confirmed"   count={counts["CONFIRMED"]   ?? 0} dot="bg-blue-400"    active={statusFilter === "CONFIRMED"}    onClick={() => changeStatus("CONFIRMED")} />
        <StatCard label="In progress" count={counts["IN_PROGRESS"] ?? 0} dot="bg-orange-400"  active={statusFilter === "IN_PROGRESS"}  onClick={() => changeStatus("IN_PROGRESS")} />
        <StatCard label="Completed"   count={counts["COMPLETED"]   ?? 0} dot="bg-green-400"   active={statusFilter === "COMPLETED"}    onClick={() => changeStatus("COMPLETED")} />
        <StatCard label="Cancelled"   count={counts["CANCELLED"]   ?? 0} dot="bg-red-400"     active={statusFilter === "CANCELLED"}    onClick={() => changeStatus("CANCELLED")} />
        <StatCard label="Delivered"   count={counts["DELIVERED"]   ?? 0} dot="bg-emerald-400" active={statusFilter === "DELIVERED"}    onClick={() => changeStatus("DELIVERED")} />
      </div>
      ...
      <div className="bg-white/40 backdrop-blur-sm border border-white/50 rounded-2xl px-4 py-3 flex items-center gap-3">
        ...
        />
        {search && (
          <button onClick={() => changeSearch("")} className="text-xs text-gray-400 hover:text-gray-600 transition-colors shrink-0">
            Clear
          </button>
        )}
        <div className="w-px h-5 bg-gray-200 shrink-0" />
        <Select value={typeFilter} onChange={(v) => changeType(v as TypeFilter)}>
                ...
                {visiblePages.map((n, i) => {
                  ...
                  return (
                    <span key={n} className="flex items-center gap-1">
                      {showGap && <span className="px-1 text-gray-300 text-sm select-none">…</span>}
                      <button
                        onClick={() => setPage(n)}
                        className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                          n === page ? "bg-black text-white" : "text-gray-600 hover:bg-gray-100"
                        }`}
                      >
                        {n + 1}
                      </button>
